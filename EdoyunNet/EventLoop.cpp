...
EventLoop::EventLoop(uint32_t num_threads) : index_(1), num_threads_(num_threads)
{
    this->Loop();
}
...
TimerId EventLoop::AddTimer(const TimerEvent &event, uint32_t mesc)
{
    ...
    {
        task_schdulers_[0]->AddTimer(event, mesc);
    }
    return 0;
}
...
void EventLoop::RemoveTimer(TimerId TimerId)
{
    if (task_schdulers_.size() > 0)
    {
        return task_schdulers_[0]->RemoveTimer(TimerId);
    }
}
...
void EventLoop::Loop()
{
    ...
    {
        std::shared_ptr<TaskScheduler> task_schduler_ptr(new EpoolTaskScheduler(i));
